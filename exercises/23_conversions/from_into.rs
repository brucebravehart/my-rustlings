...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        let parsed_vec = s.split(',').collect::<Vec<&str>>();
        if parsed_vec.len() != 2 {
            Person::default()
        } else {
            if parsed_vec[0].is_empty() {
                Person::default()
            } else {
                if parsed_vec[1].parse::<u8>().is_err() {
                    Person::default()
                } else {
                    Person {
                        name: parsed_vec[0].to_string(),
                        age: (parsed_vec[1].parse::<u8>().unwrap()),
                    }
                }
            }
        }
